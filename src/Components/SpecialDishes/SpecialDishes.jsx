...
export default function SpecialDishesSlider() {
  return (
    <section className="max-w-6xl mx-auto px-4 sm:px-2 lg:px-4 py-12">
      ...
      >
        {dishes.map((dish, idx) => (
          <SplideSlide key={idx}>
            <div className="relative bg-white shadow-md hover:shadow-lg transition rounded-xl p-4 sm:p-10 flex flex-col items-center text-center h-full">
              ...
              <img
                src={dish.image}
                alt={dish.title}
                className="w-32 h-32 sm:w-40 sm:h-40 object-cover rounded-full shadow mb-4"
              />
              <h3 className="font-semibold text-lg sm:text-xl mb-1">{dish.title}</h3>
              <p className="text-sm sm:text-base text-gray-500">{dish.description}</p>
            </div>
          </SplideSlide>
        ))}
      </Splide>
